...
const additionalMentors = [
  {
    ...
    faq: ['품질안전 직무의 점검 루틴', '위생 기준을 서류에 쓰는 방법', '신입에게 요구되는 문서화 역량'],
  },
  {
    mentor: '최다은',
    company: 'CJ제일제당',
    job: '품질보증',
    score: 83,
    summary: '식품 품질보증 직무에서 기준서 검토, 클레임 대응 자료 정리, 내부 점검 지원 업무를 맡고 있습니다.',
  ...
  },
  {
    ...
    job: '생산품질',
    score: 81,
    summary: '생산품질 파트에서 공정 데이터 확인, 생산 이슈 기록, 품질 편차 분석을 지원하고 있습니다.',
    advice: '생산 현장 경험이 없더라도 실험 데이터 정리 습관은 생산품질 직무에서 충분히 강점이 됩니다.',
  ...
  },
  {
    ...
    job: '식품안전 분석',
    score: 79,
    summary: '식품안전 데이터 분석과 기준 검토 업무를 하며, 공공기관 성격의 식품안전 지원 업무를 수행하고 있습니다.',
    advice: '직무 선택 폭을 넓히려면 품질관리뿐 아니라 식품안전 행정·분석 직무도 함께 보는 것이 좋습니다.',
  ...
  },
  {
    ...
    job: '음료 품질관리',
    score: 78,
    summary: '음료 제품 품질관리 업무에서 샘플 검사, 품질지표 확인, 생산 이슈 모니터링을 담당하고 있습니다.',
    advice: '음료 직무 지원 시에는 미생물과 안정성 평가 경험을 한 묶음으로 정리하는 것이 효과적입니다.',
...
export function PivotLabPage() {
  ...
  const mentorCandidates = [minseoMentor, mentorProfiles[1], mentorProfiles[2], ...additionalMentors]
  ...
  const pagedMentors = mentorCandidates.slice(mentorPage * MENTORS_PER_PAGE, (mentorPage + 1) * MENTORS_PER_PAGE)
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-[#64748b]">
                <span className="rounded-full border border-[#c9d9f8] bg-white px-3 py-1 font-semibold text-[#3a5fd9]">총 {mentorCandidates.length}명 추천</span>
                <span>{mentorPage + 1} / {totalMentorPages} 페이지</span>
            ...
            return (
              <div key={mentor.mentor} className="overflow-hidden">
                ...
                >
                  ...
                  <div className="min-w-0 text-sm text-[#64748b]">{mentor.company} · {mentor.job}</div>
                  <div className="min-w-0 text-sm text-[#64748b] truncate">{mentor.summary}</div>
                  <div className="flex justify-center">
                    <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${mentorBadgeTone(mentor.score)}`}>
                      {mentor.score}%
                    </span>
                  </div>
                  <div className="flex justify-center">
                      ...
                      <div className="rounded-xl border border-[#dce4f3] bg-white p-5 shadow-sm">
                        ...
                        <div className="mt-4 rounded-lg bg-[#f3f6fb] p-3 border border-dashed border-[#c9d9f8]">
                          <p className="text-[11px] leading-5 text-[#647387]">
                            AI가 현재 학생의 목표 직무와 가장 가까운 선배 질문 주제를 먼저 정리했습니다. 필요한 항목만 확인한 뒤 상담 질문 초안으로 바로 옮길 수 있습니다.
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            {pageNumbers.map((pageNumber, index) => {
              ...
              return (
                <div key={pageNumber} className="flex items-center gap-1.5">
                  {showEllipsis ? <span className="px-1 text-[11px] text-[#8a9ab5]">...</span> : null}
                  <button
                    type="button"
