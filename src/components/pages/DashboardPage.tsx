...
const roadmapBoxes = [
  ...
  { title: '목표기업 유사여부', status: '희창유업 96%', badge: 'MATCH', icon: '🔍', ai: '품질관리 직무 적합도 높음' },
  { title: '맞춤 채용정보', status: '8건', icon: '💼', ai: 'AI 추천 공고 실시간' },
  { title: '이력서 작성', status: '1차 초안 진행중', icon: '📝', ai: '품질관리 자소서 문항 설계' },
  { title: '나의 희망진로현황', status: '식품 품질·연구', icon: '🎯', ai: '부산·서울 기업 중심 탐색' },
]
...
const COURSE_TIMESTAMP = '2026-03-27 11:15:48 기준'
...
export function DashboardPage() {
  ...
  const pagedCourses = semesterCourses.slice(coursePage * COURSES_PER_PAGE, (coursePage + 1) * COURSES_PER_PAGE)
      ...
      <div className="grid gap-4 lg:grid-cols-2 lg:items-start">
        ...
        <section className="rounded-md border border-[#d6def0] bg-white shadow-sm overflow-hidden flex flex-col lg:min-h-[420px]">
          <div className="border-b border-[#e4e9f2] px-5 py-3 text-[17px] font-semibold text-[#37455e] bg-[#f8fafd] flex items-center justify-between gap-3">
            ...
            <button type="button" className="text-[11px] font-semibold text-[#5b86f7] hover:text-[#3a5fd9]">
              더보기&gt;
            </button>
          </div>
          <div className="p-5 flex-1 flex flex-col">
            <div className="grid grid-cols-4 gap-2">
              {completionStatus.map((item) => (
                <div key={item.label} className="rounded-md border border-[#e4e9f2] bg-[#f8fafd] px-3 py-3 text-center">
                  <div className="text-[10px] text-[#7b8597] truncate">{item.label}</div>
                  <div className={`mt-1.5 text-[15px] font-bold ${item.tone} whitespace-nowrap`}>{item.value}</div>
                </div>
              ))}
            </div>
            <div className="mt-4 rounded-md border border-[#e4e9f2] bg-white overflow-hidden">
              ...
              <div className="divide-y divide-[#eef2f7]">
                {pagedCourses.map((course) => (
                  <div key={course.title} className="grid grid-cols-[72px_1fr_64px_56px] items-center px-4 py-3 text-sm text-[#41516a]">
                    <span className="text-[11px] font-medium text-[#7b8597]">{course.category}</span>
                ...
                {Array.from({ length: totalCoursePages }).map((_, i) => (
                  <button
                    ...
                    className={`rounded-full transition-colors ${
                      i === coursePage ? 'h-2 w-2 bg-[#3a5fd9]' : 'h-1.5 w-1.5 bg-[#d0d9ea] hover:bg-[#a0b0cc]'
                    }`}
                  />
                ))}
              </div>
              <span className="text-[10px] text-[#8a9ab5]">{COURSE_TIMESTAMP}</span>
            </div>
          </div>
        </section>
      </div>
          ...
          <section className="rounded-md border border-[#d6def0] bg-white overflow-hidden shadow-sm">
            ...
            <div className="p-4 space-y-3">
              {campusNotices.map((notice) => (
                <div key={`${notice.title}-${notice.date}`} className="rounded-xl border border-[#e8eef6] bg-[#f8fafd] px-4 py-3 hover:border-[#cfd9eb] hover:bg-white transition-colors">
                  <div className="flex items-center justify-between gap-3">
                    <span className="rounded bg-[#e7eefb] px-2 py-0.5 text-[10px] font-bold text-[#3a5fd9]">{notice.tag}</span>
