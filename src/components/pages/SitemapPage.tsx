...
const sitemapGroups = [
  ...
  { title: '취업자료·기업분석', items: ['취업자료실', '기업 분석 리포트', '직무 참고자료', '기업정보'] },
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between gap-2 rounded-lg px-3 py-2 transition-colors ${
                        isRecommended ? 'bg-[#eef2fb] border border-[#c9d9f8] font-semibold text-[#3a5fd9]' : 'hover:bg-white/50'
                      }`}
                    >
                      <span>{item}</span>
                      {isRecommended && (
                        <span className="rounded-full bg-[#3a5fd9] px-1.5 py-0.5 text-[9px] font-bold text-white uppercase">AI Next</span>
                      )}
                    </div>
                  )
                })}
