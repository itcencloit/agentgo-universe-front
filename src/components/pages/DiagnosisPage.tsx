...
const fitReportSections = [
  {
    title: '학점 및 교과 이수',
    summary: '전공 평점과 핵심 교과 이수 흐름이 품질관리 직무의 기본 적합도를 안정적으로 지지합니다.',
    items: [
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[0.96fr_1.04fr]">
          <div className="rounded-lg border border-[#dce4f3] bg-[#f8fbff] p-5">
            <div className="flex items-center justify-between gap-3">
              <div className="text-xs font-semibold uppercase tracking-wide text-[#3a5fd9]">직무 적합도 분석</div>
              <button
                type="button"
            ...
            <div className="mt-4 flex flex-col items-center">
              <div className="relative flex h-44 w-44 items-center justify-center rounded-full bg-[conic-gradient(#316bff_0_94%,#8cb4ff_94%_88%,#dbe7ff_88%_100%)] p-4">
                <div className="flex h-full w-full flex-col items-center justify-center rounded-full bg-white text-center">
                  <div className="text-[11px] font-semibold uppercase tracking-wide text-[#7d8798]">Best Fit</div>
              ...
              <div className="mt-5 grid w-full gap-2">
                {[
                  ...
                  <div key={item.label} className="flex items-center justify-between rounded-lg border border-[#e8eef6] bg-white px-4 py-3">
                    <span className="text-sm font-semibold text-[#344257]">{item.label}</span>
                    <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${item.tone}`}>{item.score}</span>
                  </div>
                ))}
              </div>
          ...
          <div className="grid gap-4">
            <div className="rounded-lg bg-white border border-[#dce4f3] p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div className="text-xs font-semibold text-[#2e7d32] uppercase tracking-wide">강점</div>
                <button
                  type="button"
              ...
              <p className="mt-3 text-xs text-[#64748b] leading-5">
                식품위생학및종합설계, 식품품질평가및종합설계, 식품미생물학실험 이수 이력이 위생 기준 해석, 품질지표 확인, 실험 기반 원인 점검 경험으로 연결돼
                품질관리 직무 설명력이 높은 상태입니다.
              </p>
            </div>

            <div className="rounded-lg bg-white border border-[#dce4f3] p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div className="text-xs font-semibold text-[#f55d78] uppercase tracking-wide">취약점</div>
                <button
                  type="button"
              ...
              <p className="mt-3 text-xs text-[#64748b] leading-5">
                식품기사 미취득, 캡스톤 결과 미정리, 품질관리 자소서 초안 부재로 인해 전공 강점을 실제 지원서와 포트폴리오로 전환하는 준비가 아직 부족합니다.
              </p>
            </div>
          </div>
        </div>
          ...
          {recommendedActions.map((item) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-bold ${item.badgeTone}`}>
                  {item.priority}
                </span>
              </div>
              ...
              <div className="mt-4 rounded-lg border border-[#e8eef6] bg-[#f8fafd] px-3 py-3">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-[10px] font-semibold uppercase tracking-wide text-[#7d8798]">AI 판정 근거</span>
                </div>
              ...
              <div className="mt-4 flex items-start justify-between gap-3 text-xs">
                ...
                <span className="text-right leading-5 text-[#2e7d32] font-semibold">{item.impact}</span>
              </div>
            </div>
          ))}
        </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <h2 className="mt-3 text-[26px] font-black tracking-tight text-[#2b3954]">
                      {openAreaDetail === 'strength' ? '직무 적합 강점 요약' : '지원 준비 취약점 요약'}
                    </h2>
            ...
            <div className="px-7 py-6">
              <div className="rounded-[28px] border border-[#dce4f3] bg-gradient-to-br from-white via-[#fbfcff] to-[#f4f7fd] p-6 shadow-[0_10px_24px_rgba(30,45,69,0.06)]">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className={`shrink-0 rounded-2xl border px-4 py-3 shadow-[0_8px_18px_rgba(30,45,69,0.05)] ${
                    openAreaDetail === 'strength'
                    ...
                    <div className={`mt-1 text-[22px] font-black tracking-tight ${openAreaDetail === 'strength' ? 'text-[#2e7d32]' : 'text-[#f55d78]'}`}>
              ...
              <div className="mt-5 grid gap-3">
                {areaDetailContent[openAreaDetail].bullets.map((item, index) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-[11px] font-black shadow-[0_6px_14px_rgba(30,45,69,0.06)] ${
                        openAreaDetail === 'strength'
              ...
              <div className="flex items-start justify-between gap-6">
                <div className="flex items-start gap-5">
                  ...
                  <div>
                    ...
                    <h2 className="mt-3 text-[30px] font-black tracking-tight text-[#2b3954]">품질관리 직무 상세 리포트</h2>
                    <p className="mt-2 max-w-2xl text-[14px] font-medium leading-6 text-[#6a768a]">
                      학점, 전공 이수, 프로젝트 경험, 자격 준비 상태를 종합해 현재 직무 정합도를 계산한 결과입니다.
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
                <div className="space-y-5">
                  <section className="rounded-[28px] border border-[#dce4f3] bg-gradient-to-br from-white via-[#fbfcff] to-[#f4f7fd] p-6 shadow-[0_12px_28px_rgba(30,45,69,0.06)]">
                    <div className="flex items-start justify-between gap-5">
                      <div>
                        <div className="text-[10px] font-black uppercase tracking-[0.18em] text-[#7a8699]">종합 판정</div>
                        <div className="mt-3 text-[34px] font-black tracking-tight text-[#1e2d45]">품질관리</div>
                        <p className="mt-3 max-w-md text-[14px] leading-6 text-[#5c697d]">
                          교과 이수 흐름과 실험·기록형 활동이 품질관리 핵심 역할과 가장 높은 정합도를 보입니다.
                        </p>
                      </div>
                      <div className="rounded-[24px] border border-[#c9d9f8] bg-white px-5 py-4 text-center shadow-[0_10px_22px_rgba(58,95,217,0.1)]">
                        <div className="text-[10px] font-black uppercase tracking-[0.16em] text-[#7a8699]">Best Fit</div>
                        <div className="mt-2 text-[40px] font-black tracking-tight text-[#316bff]">94%</div>
                      </div>
                    </div>
                    ...
                    <div className="mt-5 grid gap-3 md:grid-cols-3">
                      {[
                        ...
                        >
                          <div className="text-[11px] font-black uppercase tracking-[0.14em] text-[#7a8699]">{item.label}</div>
                          <div className={`mt-2 text-[24px] font-black tracking-tight ${item.active ? 'text-[#316bff]' : 'text-[#344257]'}`}>{item.score}</div>
                        </div>
                      ))}
                    </div>
                  </section>
                  ...
                  <section className="rounded-[28px] border border-[#dce4f3] bg-white p-6 shadow-[0_10px_24px_rgba(30,45,69,0.05)]">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="text-[10px] font-black uppercase tracking-[0.18em] text-[#7a8699]">분석 지표</div>
                        <div className="mt-2 text-[18px] font-black tracking-tight text-[#344257]">핵심 입력 데이터</div>
                      </div>
                    ...
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                      {fitEvidence.map((item) => (
                        <div key={item.label} className="rounded-2xl border border-[#e8eef6] bg-[#fbfcff] p-4">
                          <div className="text-[11px] font-black uppercase tracking-[0.14em] text-[#7a8699]">{item.label}</div>
                          <div className="mt-2 text-[22px] font-black tracking-tight text-[#344257]">{item.value}</div>
                          <div className="mt-1 text-[12px] leading-5 text-[#64748b]">{item.note}</div>
                        </div>
                      ))}
                    </div>
                  </section>
                ...
                <div className="space-y-5">
                  {fitReportSections.map((section, index) => (
                    <section key={section.title} className="rounded-[28px] border border-[#dce4f3] bg-white p-6 shadow-[0_10px_24px_rgba(30,45,69,0.05)]">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <div className="text-[10px] font-black uppercase tracking-[0.16em] text-[#7a8699]">핵심 인사이트</div>
                          <div className="mt-2 text-[18px] font-black tracking-tight text-[#344257]">{section.title}</div>
                          <p className="mt-2 text-[14px] leading-6 text-[#64748b]">{section.summary}</p>
                        </div>
                      </div>
                      <div className="mt-4 space-y-2.5">
                        {section.items.map((item) => (
                          <div key={item} className="rounded-2xl border border-[#e8eef6] bg-[#fbfcff] px-4 py-3.5 text-[14px] leading-6 text-[#556276]">
                            {item}
                          </div>
                        ))}
                      </div>
                  ...
                  <section className="rounded-[28px] border border-[#c9d9f8] bg-gradient-to-br from-[#f6f9ff] to-[#eef3ff] p-6 shadow-[0_12px_26px_rgba(58,95,217,0.08)]">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="text-[10px] font-black uppercase tracking-[0.18em] text-[#7a8699]">추천 후속 액션</div>
                        <div className="mt-2 text-[18px] font-black tracking-tight text-[#344257]">다음 단계 제안</div>
                      </div>
                    ...
                    <div className="mt-4 space-y-2.5">
                      {[
                        ...
                        <div key={item} className="rounded-2xl border border-white/80 bg-white/80 px-4 py-3.5 text-[14px] leading-6 text-[#556276] shadow-[0_6px_16px_rgba(30,45,69,0.04)]">
                          {item}
                        </div>
                      ))}
                    </div>
