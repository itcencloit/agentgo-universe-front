...
export const transferableSkills: TransferableSkill[] = [
  {
    ...
    evidence: '식품화학 실험 6회 이상 수행, 성분 분석 데이터를 바탕으로 원인 가설 수립·검증. 전공 평균 4.12/4.5로 이론 기반 탄탄.',
    interviewTip: '"실험 데이터에서 이상값 발견 → 원료 로트 추적 → 원인 특정" 흐름으로 서술하면 품질관리 직무와 직결됩니다. 분석 횟수·오차율 개선 % 등 수치를 포함하세요.',
    consultingCheck: [
  ...
  },
  {
    ...
    evidence: '식품미생물학 A 이수, 위생관리 실험 실습 포함. HACCP 7원칙 이론 학습 완료 및 실제 CCP 설정 실습 경험 보유.',
    interviewTip: '"미생물 오염 경로 파악 → CCP 설정 → 모니터링 기준 제안" 구조로 답하면 현업 투입 가능한 인상을 줍니다. 실험 중 검출·판정 경험을 구체적으로 연결하세요.',
    consultingCheck: [
  ...
  },
  {
    ...
    evidence: '식품 제조 공정 수율 저하 문제를 팀 단위로 분석하고 개선안 도출. 개선 전·후 수율 비교 데이터 정리 필요.',
    interviewTip: '"문제 정의 → 원인 분석 → 개선안 제안 → 결과 검증" 4단계 구조로 서술하고 수율 개선율·불량률 감소 수치를 반드시 포함하세요. 자소서 3번 문항과 직결됩니다.',
    consultingCheck: [
